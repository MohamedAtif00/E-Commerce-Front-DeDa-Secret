...
export class ProductDetailsComponent implements OnInit {
  ...
  GetProduct(id: string): void {
    this.productService
      ...
      .pipe(
        switchMap((productData) => {
          ...
          const imageRequests$ = this.product.images
            ...
            .map((image) =>
              this.productService
                .getProductImage(this.product.id, image.id)
                .pipe(
                  catchError((error) => {
        ...
        map(([masterImageBlob, categoryData, ...productImageBlobs]) => {
          ...
          const masterImageUrl = masterImageBlob
            ? this.convertBlobToImageUrl(masterImageBlob)
            : '';
          this.productService.file.set(new File([masterImageBlob], 'image'));

          // Process product images
          const uploadedFiles = productImageBlobs
            .filter((blob) => blob)
            .map((blob, index) => {
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error(
            'Error occurred while processing product details:',
            err
          );
        },
      });
  ...
  UpdateProduct(): void {
    const updateProductSubscription = this.productService
              ...
              return forkJoin(uploadObservables).pipe(
                switchMap(() => {
                  ...
                  if (this.productService.file()) {
                    ...
                    masterImageFormData.append(
                      'file',
                      this.productService.file()
                    );
